# -*- coding: utf-8 -*-
"""
Created on Wed Apr 05 11:36:26 2017
...
# ------------------ Disconnecting individual line(s) from the subset ------------------
        for j in range(0, size_of_contingency):
            ...
            for each_stage_vector in range(0, len(stage_outage_vec_temp)):
                initial_loss = [];
                identify_initial_outage_state_vec = [];
                stagecounter = 0;
...
#                        DSSCircuit.CktElements(contingency[j][k]).Open(2,0);
                        ...
                        identify_initial_outage_state_vec.append(stage_vec[k]);
                print "Initial outage: %s" %initial_loss
#                for item in range(0, len(stage_outage_vec_temp)):
#                    if (stagecounter == stage_outage_vec_temp[item]):
##                    additional_outage =  'Line.tl2122';
#                        initial_loss.append(contingency[j][item]);
#                        identify_initial_outage_state_vec.append(stage_outage_vec_temp[item])
#                        print '*******************************************************************************'
#                        print 'Additional initial level outage in stage {0}: {1}'.format(stagecounter, additional_outage)
#                        print '*******************************************************************************'
#                        DSSCircuit.CktElements(contingency[j][item]).Open(1,0);
#                        DSSCircuit.CktElements(contingency[j][item]).Open(2,0);
...
#                            DSSCircuit.CktElements(contingency[j][k]).Open(2,0);
                            ...
                            identify_initial_outage_state_vec.append(stage_vec[k]);
                            print '@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@'
                            print 'Additional Initial Outage in stage {0}: {1}'.format(stagecounter, contingency[j][k])
                            print '@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@'
...
#                        DSSCircuit.CktElements(additional_outage).Open(2,0);
                        DSSText.Command = "Open " + additional_outage + " 1"
                        DSSText.Command = "Open " + additional_outage + " 2"
...
#    print "Number of Blackout cases = %d" %blackout_counter
    max_load_loss_outage = temp_first_outage;
    max_load_loss_outage.append(temp_additional_dynamic_outage);
    max_load_loss_outage_vec = temp_first_outage_state_vec;
    max_load_loss_outage_vec.append(temp_additional_dynamic_outage_state_vec);    
    total_execution_time_end = time.time();
    total_execution_time = (total_execution_time_end - total_execution_time_start)
    print '############################################################################'
    print 'Max Load Loss Causing Outage: %s' %max_load_loss_outage
    print 'Max Load Loss Causing Vector: %s' %max_load_loss_outage_vec
    print 'Max Load Loss (in percent): %f' %max_load_loss
    print 'Total Number of Simulation runs: %d' %Total_sim_counter
    print 'Total execution time in seconds: %s' %total_execution_time
    print '############################################################################'
    return (max_load_loss, max_load_loss_outage, max_load_loss_outage_vec);
#    ------------------------------------------------------------------------------------            
